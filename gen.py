...
def sol_error_selector(function_name, param_names, param_types):
    ...
    selector_comment = f'// bytes4(keccak256("{function_name}({",".join(param_types)})"))'
...
# todo: bytes memory params
def sol_error(function_name, param_names, param_types, selector_name):
    param_types = [type + ' memory' if type == 'bytes' else type for type in param_types]
...
def ts_error(function_name, param_names, param_types):
    ...
    ts_params = [f'{name}?: {ts_type(sol_type)}' for (name, sol_type) in zip(param_names, param_types)]
    ...
    super_string = f'super({",".join([name_string, declaration_string, values_string])});'
    constructor_string = f'constructor({ts_params_string}) {{ {super_string} }}'
    class_string = f'export class {function_name} extends RevertError {{ {constructor_string} }}'
...
def sol_codegen(lib_name, functions, target):
    ...
    for function_name, function_obj in functions.items():
        ...
        selector_name, selector_comment, selector_string = sol_error_selector(function_name, param_names, param_types)
        error_selectors.append(f'{selector_comment}\n{selector_string}')
        error_functions.append(sol_error(function_name, param_names, param_types, selector_name))

    shutil.copyfile('sol_prefix.txt', target)
...
def ts_codegen(functions, repo, target):
    ...
    for function_name, function_obj in functions.items():
        ...
        selector_name, _, _ = sol_error_selector(function_name, param_names, param_types)
        error_classes.append(ts_error(function_name, param_names, param_types))

    shutil.copyfile('ts_prefix.txt', target)
    with open(target, 'a') as f:
        f.write('\n\n')
        f.write('\n\n'.join(error_classes))
        f.write(f'\n\nconst types = [{",".join(functions.keys())}];\n')
        with open('ts_suffix.txt', 'r') as file_suffix:
...
sol_target = f'{repo}/{sol_path}'
ts_target = f'{repo}/{ts_path}'
...
ts_codegen(contractObject.functions, repo, ts_target)
